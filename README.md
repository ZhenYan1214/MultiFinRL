...
| Price-movement label | 5-trading-day forward return; train-only 1/3 and 2/3 quantiles define BEARISH / NEUTRAL / BULLISH |
...
> Evaluation protocol was tightened on 2026-09-19: train-only label thresholds/fusion/PPO,
> purged boundary labels, a single persisted split, test-only final evaluation, and artifact
> fingerprints are now enforced. Metrics listed below are historical engineering results until the
> A→B→C pipeline is retrained under this protocol; do not present them as new held-out results.
