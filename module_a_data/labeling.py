...
定義（2026-08 改為分位數門檻，取代 docs/decisions.md #16 的固定 ±2%，見 #30）：
    ...
    只用 train split 的「每一天 r」分布，取其 1/3、2/3 分位數當門檻，再固定套用全期間：
    r > 分位數門檻(高)  -> BULLISH
    r < 分位數門檻(低)  -> BEARISH
    其餘                -> NEUTRAL
    這樣三類天數會自然接近平衡（各約 1/3），門檻也會隨資料實際波動幅度調整，
    不像固定 2% 可能剛好切在報酬分布最密集的地方，製造大量邊界模糊天。
    門檻數值只從 train 期間估計，避免 validation/test 報酬分布滲入標籤規則。
...
def compute_quantile_thresholds(df: pd.DataFrame, horizon: int = 5,
                                low_q: float = 1 / 3, high_q: float = 2 / 3,
                                fit_dates: list[str] | None = None) -> tuple[float, float]:
    ...
    if fit_dates is not None:
        wanted = pd.to_datetime(fit_dates)
        r = r.loc[r.index.intersection(wanted)]
        if r.empty:
            raise ValueError("train split 沒有可用報酬，無法計算分位數門檻")
...
def build_price_and_label(df: pd.DataFrame, date: str, horizon: int,
    ...
    對 train split 算好、傳進來的固定值（見 build_dataset.py），每一天共用同一組門檻。
    ...
    return {
        "prices": {"close_t0": round(close_t0, 4), "close_t5": close_t5,
                   ...
                   "target_date": df.index[idx + horizon].strftime("%Y-%m-%d")},
