...
6. Extract notable financial events from news, filings, and transcripts, and evaluate against ground truth with precision / recall / F1 (`event_extraction.py`, keyword or LLM-based method via `--method`; this is a standalone analysis of A's data, separate from the H_v/H_t/H_r/Z_fused pipeline — it does not feed into steps 1-4 or 7).
...
- Event extraction has two methods, switchable with `--method` (`event_extraction.py`). `keyword` (default, free, `EVENT_KEYWORDS`) has no semantic understanding; measured on AAPL against the 149-day ground truth: precision=0.162, recall=0.868, f1=0.273 — recall is high (rarely misses a real event) but precision is low (plenty of false positives, e.g. it can't reliably tell "discussing" something apart from "announcing" it that day). See `docs/decisions.md` #32 for the full before/after breakdown and root-cause analysis. `llm` calls an LLM API per day (`--provider claude/openai/deepseek`, shared calling logic in `llm_client.py`) using the same prompt and judging standard as the ground truth labeling; supports resumable caching and a `--limit_to_ground_truth` flag to validate cheaply on the 149-day sample before spending on a full run. Design: `docs/spec_b_event_extraction_llm.md`. **Measured on AAPL with `--provider deepseek` on the 149-day sample: precision=0.742, recall=0.605, f1=0.667** — a large improvement over keyword (f1 +144%); precision is far better (the LLM rarely over-triggers) but recall dropped (it's more conservative and misses some real events, notably LAWSUIT: 21/36 caught, and the single MA day: missed entirely). See `docs/decisions.md` #35 for the full per-category breakdown. Full 1381-day run decided and in progress on AAPL. `module_b_encoder/event_ground_truth_llm.py` and `event_extraction.py --method llm` share the same `llm_client.py`, so ground truth labeling and production extraction stay consistent.
