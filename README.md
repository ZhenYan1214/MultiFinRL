...
時程已由原本兩年壓縮為一年，原「第一年／第二年」的內容全部併入同一年執行，改以「第一階段／第二階段」劃分（見 `docs/decisions.md` #18）。

| 階段 | 目標 |
|---|---|
| 第一階段 | 建立完整資料與模型 pipeline，產出 Z_fused，以**市場情緒分類與事件抽取**驗證其品質（本年度原始驗證目標，不可忘記），並完成簡單的投資組合回測（單股比例增減或多標的組合） |
| 第二階段 | 將 Z_fused 接上 RL（PPO），完成投資組合配置與回測整合，並**比較加入 RL 前後的績效／metric 變化**（Sharpe Ratio、最大回撤等） |

兩階段在同一年內接續進行，pipeline 需完整涵蓋兩階段的產出（分類驗證 + 事件抽取 + 簡單回測 + RL 回測），不是分年度才做。
