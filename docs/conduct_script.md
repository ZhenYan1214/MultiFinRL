...
# 抓調整後股價，存成 AAPL.csv + AAPL.meta.json（日期範圍與 auto_adjust 讀 config）
...
# 漲跌標籤分位數門檻只依 train 報酬估計；會寫 manifest 與固定 split，跨界標籤自動 purge
...
# 第一次先測：寫到 data/vectors/AAPL_FAKE，不會蓋掉正式向量
...
# 報告存到 data/outputs/metrics/event_extraction_report_AAPL.json
...
# 第一次先測：用假向量測架構，checkpoint 存 fusion_fake.pt，不會蓋正式權重
...
# 用 Z_fused 做情緒分類驗證，準確率存到 data/outputs/metrics/classification_report_AAPL.json
...
# 只用 train split 訓練 PPO，存到 data/outputs/checkpoints/ppo_agent_AAPL.zip
# --curriculum 開啟 curriculum learning（由連續低波動區間逐步擴張到完整 train）

py -m module_c_fusion.explainability.integrated_gradients --ticker AAPL
# 對訓練好的 PPO policy 做跨模態歸因（captum Integrated Gradients），
# 需要先有 Z_fused 跟 ppo_agent_AAPL.zip；只解釋 held-out test 決策

py -m module_c_fusion.backtest.backtest --ticker AAPL --strategy buy_and_hold
# 回測策略一：只在 held-out test 區間滿倉（baseline 對照組）
